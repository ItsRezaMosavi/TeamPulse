...
public sealed class EfSpecificationEvaluator : ISpecificationEvaluator
{
	...
	///     <list type="number">
	///         <item>
	///             <description>WHERE clause from Criteria if present</description>
	///         </item>
	///         <item>
	///             <description>INCLUDE statements for related data</description>
	///         </item>
	///         <item>
	///             <description>ORDER BY or ORDER BY DESCENDING based on IsAscending</description>
	///         </item>
	///         <item>
	///             <description>SKIP and TAKE for pagination if enabled</description>
	///         </item>
	///     </list>
	/// </remarks>
	public IQueryable<T> GetQuery<T>(IQueryable<T> query, IQuerySpecification<T> specification) where T : class
	{
		query = ApplyQuery(query, specification);
		return query;
	}

	public IQueryable<TResult> GetQuery<T, TResult>(IQueryable<T> query, IQuerySpecification<T, TResult> specification)
		where T : class
	{
		query = ApplyQuery(query, specification);
	...
	private static IQueryable<T> ApplyQuery<T>(IQueryable<T> query,
											   IQuerySpecification<T> specification) where T : class
	{
		...
		query = specification.IsSplitQuery ? query.AsSplitQuery() : query.AsSingleQuery();

		foreach (var include in specification.Includes) query = query.Include(include);
